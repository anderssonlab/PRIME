...
corrProfiles <- function(object, pooled, flank = 300, lag.max = 300) {
  assertthat::assert_that(
    methods::is(object, "GRanges"),
    "thick" %in% colnames(S4Vectors::mcols(object)),
    methods::is(pooled, "RangedSummarizedExperiment"),
    is.numeric(flank), length(flank) == 1, flank >= 0,
    is.numeric(lag.max), length(lag.max) == 1, lag.max >= 0
  )
