...
#' @export
#'
#' @importFrom GenomicRanges GRanges seqnames strand
...
cumulativeFractionAroundLoci <- function(loci, ctss, max_dist = 1000) {
  ...
  p <- GenomicRanges::GRanges(seqnames = GenomicRanges::seqnames(loci), loci$thick, strand = "+")
  m <- GenomicRanges::GRanges(seqnames = GenomicRanges::seqnames(loci), loci$thick, strand = "-")
  t <- methods::as(SummarizedExperiment::rowRanges(ctss), "GRanges")

  message("findining nearest loci strand-wise...")
  p_nearest <- IRanges::follow(t, p, select = "last")
  m_nearest <- IRanges::follow(t, m, select = "last")

  message("calculating distances...")
  d <- rep(Inf, length(t))
  non.na <- which(!is.na(p_nearest))
  d[non.na] <- IRanges::distance(t[non.na], p[p_nearest[non.na]])
  non.na <- which(!is.na(m_nearest))
  d[non.na] <- IRanges::distance(t[non.na], m[m_nearest[non.na]])

  if (!"totalTags" %in% colnames(SummarizedExperiment::colData(ctss))) {
  ...
  fracs <- sapply(colnames(ctss), function(n) {
    cat(".")
    a <- as.numeric(SummarizedExperiment::assay(ctss, "counts")[o, n])
    nz <- which(a > 0)
    da <- d[nz]
    a <- a[nz]
