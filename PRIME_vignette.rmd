...
```{r}
library("devtools")
devtools::install_github("anderssonlab/PRIME@remove_PRIMEloci")
```
...
```{r}
tcs <- CAGEfightR::clusterUnidirectionally(CTSSs)

#DL functions
dl <- PRIME::divergentLoci(tcs, orig_ctss)

ctss_complexity <- PRIME::calcCTSSComplexity(orig_ctss)
